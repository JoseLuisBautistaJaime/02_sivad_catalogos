...
import mx.com.nmp.ms.sivad.catalogo.service.TipoPiedraComplementariaService
import org.crsh.cli.*
...
/**
 * Utilizada por la consola CRaSH para la administración del catálogo Tipo Piedra Complementaria
 *
 * @author roramirez
 */
@Usage("Administración del catálogo Tipo Piedra Complementaria")
class piedraComplementaria {
    ...
    def agregar(InvocationContext context,
        ...
        try {
            def tipoPiedraComp = new TipoPiedraComplementaria([abreviatura: abreviatura, etiqueta: etiqueta])
            def elemento = getServicio(context).save(tipoPiedraComp)
    ...
    def modificar(InvocationContext context,
        ...
        if (ObjectUtils.isEmpty(abreviatura) && ObjectUtils.isEmpty(etiqueta)) {
            ...
            try {
                def piedraComplem = new TipoPiedraComplementaria([abreviatura: abreviatura, etiqueta: etiqueta])
                def elemento = getServicio(context).update(abreviaturaActual, piedraComplem)
    ...
    /**
     * Permite obtener la instancia del servicio TipoPiedraComplementariaService.
     *
     * @param context El contexto de la invocación.
     * @return Referencia al servicio de TipoPiedraComplementariaService.
     */
    private static TipoPiedraComplementariaService getServicio(InvocationContext context) {
        context.attributes['spring.beanfactory'].getBean(TipoPiedraComplementariaService)
    }
