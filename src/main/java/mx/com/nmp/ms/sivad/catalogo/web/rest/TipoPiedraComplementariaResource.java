...
/**
 * Controlador REST utilizado para obtener la información del catálogo Tipo Piedra Complementaria.
 *
 * @author roramirez
 */
@RestController
@RequestMapping("/catalogos/diamantes/tipo_complemento")
public class TipoPiedraComplementariaResource {

    /**
     * Utilizada para manipular los mensajes informativos y de error.
     */
    private static final Logger LOGGER = LoggerFactory.getLogger(TipoPiedraComplementariaResource.class);

    /**
     * Referencia al servicio de tipoPiedraComplementariaService.
     */
    @Inject
    ...
    public ResponseEntity<Catalogo> getAll() {
        LOGGER.info(">> getAll");
        List<TipoPiedraComplementaria> result = tipoPiedraComplementariaService.getAll();

            if(ObjectUtils.isEmpty(result)) {
                return new ResponseEntity<>(HttpStatus.OK);
            }else {
                return new ResponseEntity<>(CatalogoFactory.build(tipoPiedraComplementariaService.getAll()), HttpStatus.OK);
            }
    }
    ...
    public ResponseEntity<Catalogo> getAll(@RequestParam("dependencias") boolean dependencias) {
        if (dependencias) {
            LOGGER.warn("El catalogo TipoPiedraComplementaria no contiene dependencias.");
