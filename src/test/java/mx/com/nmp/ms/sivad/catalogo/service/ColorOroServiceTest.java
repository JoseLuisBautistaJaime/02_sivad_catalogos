...
public class ColorOroServiceTest {
    ...
    public void testAgregarElementoCatalogo_2() {
        ...
        try {
            ...
            fail();
        } catch (CatalogoDuplicateKeyException cdke) {
            assertNotNull(cdke);
            assertEquals("No fue posible realizar el guardado de la entidad. El catalogo ColorOro ya " +
                    "contiene un elemento con la abreviatura: [" + CAT_ABREVIATURA_DEFAULT + "].",
                    cdke.getMessage());
        } catch (Exception e) {
            LOGGER.error("Ocurrio una excepcion inesperada realizar la operacion. {}", e.getMessage());
            fail();
    ...
    public void testEliminarElementoCatalogo_2() {
        ...
        try {
            ...
            fail();
        } catch (CatalogoNotFoundException cnfe) {
            assertNotNull(cnfe);
            assertEquals("El catalogo ColorOro no contiene un elemento con la " +
                    "abreviatura: [" + CAT_ABREVIATURA_NO_EXISTE + "].", cnfe.getMessage());
        } catch (Exception e) {
            LOGGER.error("Ocurrio una excepcion inesperada realizar la operacion. {}", e.getMessage());
            fail();
        }
    ...
    @Transactional
    @Sql("/bd/test-data-color-oro-h2.sql")
    public void testObtenerTodosElementosCatalogo() {
        LOGGER.info(">> testObtenerTodosElementosCatalogo");
